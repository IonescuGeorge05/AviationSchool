  ...
  async (request, reply) => {
    ...
    try {
      ...
      if (
        ...
        return reply.status(409).send({ message: "Message already exists" });
      }
  ...
  async (request, reply) => {
    ...
    try {
      ...
      if (
        ...
        return reply.status(409).send({ message: "You are already subscribed" });
      }
...
fastify.get(
  ...
  async (request, reply) => {
    const result = await fastify.sqlite.get(
      SQL`select count(email) from subscribers;`
    );
    return {count: result["count(email)"]}
  }
);
